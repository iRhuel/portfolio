module.exports = [
    ...
    },
    {
        name: 'SMUSHR',
        description: [
            `An NPM-packaged .json parsing utility to aggregate data from multiple files into a single, readable result. It
        ...
        context: [
            ...
            able to use it in their situation. For now it only aggregates data points as primitives in an array, but
            I want to redesign the tool to represent datum as an object with both the original value, and a record
            of all files that data point was represented in, so that the user can eventually see what data is relevant
            to what files, or what other data shares a common file.`,

            `It's currently publicly available as an NPM package, install with 'npm i -g smushr'`
        ],
        lessons: [
